...
| `uiNamespace` | `string` | No       | Namespace for UI design props (`{i18nScope}.import-button`).              |
| `i18nScope`   | `string` | No       | Scope for i18n translations (`{i18nScope}.ImportButton`).                 |
...
```vue
<q-btn v-bind="uiProps" :label="t('label')" class="import-button" data-cy="button_import" @click="goToImport" />
```
