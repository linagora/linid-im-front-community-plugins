...
| `options`        | `ModuleUsersOptions`                        | Configuration options for the module, including `formSections`.                                |
...
- Triggers automatic recalculation of `isDisabled` computed property.
...
3. `user` and `initialUser` are populated with the fetched data.
4. Form sections and fields are rendered based on `formSections` configuration.
5. User edits field values through `EntityAttributeField` components.
6. Each change triggers `onFieldValueChange`, updating `user` and recalculating `isDisabled`.
7. User clicks "Save" button (or presses Enter).
8. Form submission triggers `save()` method.
9. `updateEntity` API is called with current `user` data.
10. On success, positive notification is shown and user is redirected to detail page.
11. On error, negative notification is shown and user stays on edit page.
