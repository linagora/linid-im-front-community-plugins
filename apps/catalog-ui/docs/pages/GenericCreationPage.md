...
| `parentPath`     | `string`        | Yes      | Route path used to navigate back to the previous page after saving the entity, and on cancel. Supports Nunjucks template interpolation with the `entity` variable (e.g. `"/entities/{{ entity.parentId }}"`) |
...
```text
{renderString(parentPath, { entity })}/{data[idKey]}
```

The template context is the submitted entity merged with the backend response (the response takes precedence). A templated path can therefore reference both the fields typed in the form and the ones generated server-side, regardless of whether the backend echoes the full entity back:

```json
{
  "options": {
    "parentPath": "/groups/{{ entity.parentId }}/members"
  }
}
```
...
- Cancel navigation redirects the user to the path produced by rendering `parentPath` as a Nunjucks template with the current entity state as context (variable: `entity`).
- A static path (e.g. `"/applications"`) works as-is; a dynamic path can reference entity fields (e.g. `"/groups/{{ entity.parentId }}/members"`).
