...
Translations are read from the fixed `application.logout` scope, like the other entries of the profile menu.

### LogoutMenuItemProps Interface

```typescript
export interface LogoutMenuItemProps extends Pick<CommonComponentProps, 'uiNamespace'> {
  path: string;
}
```
...
2. On confirm, the component checks that `path` matches a route of the host router, then calls `router.push(path)`. The host route is responsible for the actual logout (for instance ending the OIDC session). When no route matches, or when the navigation fails, the user stays on the page and a negative notification (`error` key) is displayed.
...
All keys are resolved under the `application.logout` scope:

| Key                                      | Description                                                |
| ---------------------------------------- | ---------------------------------------------------------- |
| `label`                                  | Label of the menu entry                                    |
| `error`                                  | Notification shown when the logout route cannot be reached |
...
```json
{
  "application": {
    "logout": {
      "label": "Log out",
      "error": "Unable to reach the logout page.",
      "ConfirmationDialog": {
        "title": "Log out",
