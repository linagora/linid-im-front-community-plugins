...
- Aggregating date constraint values (`afterDate`/`beforeDate`/`fromDate`/`upToDate`), which are then normalized either to the display mask (for validation rules) or to the corelib's `QDATE_DEFAULT_MASK` (for the date-picker's available `options` predicate — see below)
...
- Validation rules applied when checking user input
- User-facing date formatting and presentation

Note: the date-picker's available `options` predicate is **not** driven by the display mask. `q-date` always calls
its `options`/`events` predicate with dates in its own internal `QDATE_DEFAULT_MASK`-shaped hash
(`year/month/day`), regardless of the configured `:mask` — so date constraint refs are normalized to
`QDATE_DEFAULT_MASK` (not `mask`) before being compared against the dates `q-date` passes in.
...
- Verify that the internal `valueFormat` is used to aggregate and store date constraint values, and is converted to `mask` before being handed to validators, and to `QDATE_DEFAULT_MASK` (not `mask`) before being handed to the date-picker's `options` predicate
