...
```ts
const options = ref(props.definition.inputSettings?.values || []);
```

- Stores the list of available values as a reactive reference
...
```ts
const defaultValue = props.definition.inputSettings?.defaultValue && options.value.includes(props.definition.inputSettings.defaultValue) ? props.definition.inputSettings.defaultValue : options.value[0];
```
...
- Available options are stored in a local `ref` for template usage
