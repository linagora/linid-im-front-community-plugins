...
describe('Test component: EntityAttributeDateField', () => {
  ...
  describe('Test computed: options', () => {
    ...
    it('should correctly filter cross-month boundary dates using QDATE_DEFAULT_MASK regardless of mask', async () => {
      // afterDate = 31 Jan 2026; the predicate must accept 1 Feb 2026 even though
      // a naive lexicographic compare in a day-first format would reject it.
      // q-date always calls options()/events() with dates in its own internal
      // QDATE_DEFAULT_MASK-shaped hash, never in the configured `mask`, so the ref
      // must be normalised to QDATE_DEFAULT_MASK, not to `mask`.
      const mockDayjsResult = { toISOString: () => '2026-01-31T00:00:00.000Z' };
      mockMaxDate.mockReturnValue(mockDayjsResult);
      mockFormatDate.mockImplementation((value, outputFormat, inputFormat) => {
        // computeRef's call (2 args): ISO -> valueFormat
        if (inputFormat === undefined) {
          return '31-01-2026';
        }
        // options' call (3 args): must target QDATE_DEFAULT_MASK, not `mask`,
        // otherwise fall through to a sentinel that can't satisfy the boundary checks below
        return outputFormat === 'YYYY/MM/DD' &&
          value === '31-01-2026' &&
          inputFormat === 'DD-MM-YYYY'
          ? '2026/01/31'
          : 'UNEXPECTED_FORMAT_DATE_CALL';
      });
      await wrapper.setProps({
        definition: {
          name: 'birthdate',
    ...
    it('should pass QDATE_DEFAULT_MASK and valueFormat to formatDate when building the options predicate, not mask', async () => {
      const mockDayjsResult = { toISOString: () => '2026-01-31T00:00:00.000Z' };
      mockMaxDate.mockReturnValue(mockDayjsResult);
      await wrapper.setProps({
        definition: {
  ...
  describe('Test function: updateValueFromInput', () => {
    ...
    it('should emit event with a null value when the input value is null', () => {
      wrapper.vm.updateValueFromInput(null);
  ...
  describe('Test reactivity: displayValue', () => {
    ...
    it('should not update displayValue when another entity attribute changes', async () => {
      const countBirthdateCalls = () =>
        mockFormatDate.mock.calls.filter((call) => call[0] === '1990/01/01')
          .length;
      const callsBefore = countBirthdateCalls();
      ...
      expect(countBirthdateCalls()).toEqual(callsBefore);
      expect(wrapper.vm.displayValue).toEqual('1990/01/01');
    });
    ...
    it('should format the entity value with a strict valueFormat parse before falling back', () => {
      const newWrapper = shallowMount(EntityAttributeDateField, {
        props: {
          uiNamespace: 'namespace',
          instanceId: 'id',
      ...
      expect(newWrapper.vm.displayValue).toEqual('2026-01-31');
    });

    it('should fall back to a format-less parse when the value does not strictly match valueFormat', () => {
      mockFormatDate.mockImplementation((value, outputFormat, inputFormat) =>
        inputFormat ? '' : `loose(${value})`
      );
      const newWrapper = shallowMount(EntityAttributeDateField, {
        props: {
          uiNamespace: 'namespace',
          ...
          entity: {
            name: 'entity-name',
            // saved before valueFormat was configured, so it doesn't match 'YYYY-MM-DD'
            birthdate: '2026/01/31',
          },
        },
        global: {
      ...
      expect(mockFormatDate).toHaveBeenCalledWith('2026/01/31', 'DD/MM/YYYY');
      expect(newWrapper.vm.displayValue).toEqual('loose(2026/01/31)');
    });
  });
