...
import NumberSearchFilterPanel from '../../../../src/components/smart-filter/NumberSearchFilterPanel.vue';
...
function mountComponent(props = {}) {
  return shallowMount(NumberSearchFilterPanel, {
...
describe('Test component: NumberSearchFilterPanel', () => {
  let wrapper;
  ...
  describe('Test function: buildFilterValue', () => {
    it('should map inferior to lt_ with the raw input value', () => {
      wrapper.vm.inputValue = '42';
    ...
    it('should map superior to gt_ with the raw input value', () => {
      wrapper.vm.inputValue = '18';
  ...
  describe('Test function: onSearch', () => {
    it('should emit search with the field name and the constructed filter value', () => {
      ...
      expect(payload.field).toBe('age');
      expect(payload.values).toHaveLength(1);
      expect(payload.values[0].operator).toBe('lt_');
      expect(payload.values[0].value).toBe('42');
    });

    it('should use the fieldName prop as the field in the payload', () => {
      wrapper = mountComponent({ fieldName: 'salary' });

      wrapper.vm.onSearch();

      expect(wrapper.emitted('search')[0][0].field).toBe('salary');
    });
  });
});
