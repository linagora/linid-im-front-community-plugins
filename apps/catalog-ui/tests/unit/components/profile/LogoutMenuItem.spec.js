...
const { mockPush, mockResolve, mockNotify, mockUiEventSubjectNext, mockUi } =
  vi.hoisted(() => ({
    mockPush: vi.fn(),
    mockResolve: vi.fn(),
    mockNotify: vi.fn(),
...
vi.mock('@linagora/linid-im-front-corelib', () => ({
  useScopedI18n: () => ({ t: (key) => `translated_${key}` }),
...
describe('Test component: LogoutMenuItem', () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    mockUi.mockImplementation(() => ({}));
    mockPush.mockResolvedValue(undefined);
    mockResolve.mockReturnValue({ matched: [{}] });

    wrapper = shallowMount(LogoutMenuItem, { props: defaultProps });
  });

  describe('Test constant: uiProps', () => {
    ...
    it('should let the design override the avatar of the icon section', () => {
      mockUi.mockImplementation((namespace, component) =>
        namespace === 'test-namespace.logout-menu-item.icon' &&
        component === 'q-item-section'
          ? { avatar: false, side: true }
          : {}
      );

      wrapper = shallowMount(LogoutMenuItem, { props: defaultProps });

      expect(wrapper.vm.uiProps.iconSection).toEqual({
        avatar: false,
        side: true,
      });
    });
  ...
  describe('Test function: openConfirmationDialog', () => {
    it('should open the confirmation dialog with the logout translations and namespaces', () => {
      ...
      expect(mockUiEventSubjectNext).toHaveBeenCalledWith({
        ...
        data: {
          type: 'open',
          title: 'translated_ConfirmationDialog.title',
          content: 'translated_ConfirmationDialog.content',
  ...
  describe('Test function: logout', () => {
    it('should navigate to the configured path', async () => {
      ...
      expect(mockResolve).toHaveBeenCalledWith('/logout');
    ...
    it('should notify an error and stay on the page when no route matches the path', async () => {
      mockResolve.mockReturnValue({ matched: [] });
    ...
    it('should notify an error when the navigation fails', async () => {
      ...
      expect(consoleError).toHaveBeenCalledWith(
        '[LogoutMenuItem] Navigation to the logout route failed',
        error
      );
