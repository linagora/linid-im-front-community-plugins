...
vi.mock('@linagora/linid-im-front-corelib', () => ({
  ...
  useLinidUiStore: () => ({ mainNavigationItems: [] }),
...
describe('Test layout: BaseLayout', () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    mockRoute.meta = reactive({});

    wrapper = shallowMount(BaseLayout);
  });

  describe('Test computed: logoutPath', () => {
    it('should default to the /logout route', () => {
      expect(wrapper.vm.logoutPath).toBe('/logout');
    });

    it('should use the route meta value', () => {
      mockRoute.meta = reactive({ logoutPath: '/bye' });
  ...
  describe('Test computed: hideLogout', () => {
    it('should show the logout entry by default', () => {
      expect(wrapper.vm.hideLogout).toBe(false);
    });

    it('should hide the logout entry when the route meta asks for it', () => {
      mockRoute.meta = reactive({ hideLogout: true });
