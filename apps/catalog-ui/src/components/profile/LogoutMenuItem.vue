...
const props = defineProps<LogoutMenuItemProps>();

const i18nScope = 'application.logout';
...
function openConfirmationDialog(): void {
  uiEventSubject.next({
    ...
    data: {
      ...
      i18nScope: `${i18nScope}.ConfirmationDialog`,
      onConfirm: logout,
    },
  });
}
...
/**
 * Navigates to the configured logout route. Notifies the user instead when the route does not exist or when the
 * navigation fails, so that a wrong configuration never leaves the user on a blank page.
 * @returns A promise that resolves once the navigation has been handled.
 */
async function logout(): Promise<void> {
  if (router.resolve(props.path).matched.length === 0) {
    console.error(
      `[LogoutMenuItem] No route matches the logout path "${props.path}"`
    );
    Notify({ type: 'negative', message: t('error') });
    return;
  }
  ...
  try {
    ...
    console.error('[LogoutMenuItem] Navigation to the logout route failed', e);
    Notify({ type: 'negative', message: t('error') });
  }
}
</script>
