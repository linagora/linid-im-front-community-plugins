    ...
    <q-card-section>
      <div class="flex entity-details-card--container">
        <information-card
          ...
          :key="field"
          :label="t(`EntityDetailsCard.attributes.${field}`)"
          :value="values[field]"
          :is-loading="isLoading"
          class="q-ma-sm"
          :ui-namespace="`${props.uiNamespace}.entity-details.${field}`"
        />
      </div>
    </q-card-section>
  </q-card>
...
const { t } = useScopedI18n(props.instanceId);
...
const uiCardProps: LinidQCardProps = ui<LinidQCardProps>(
  `${props.uiNamespace}.entity-details`,
  'q-card'
);
</script>
