...
<template>
  ...
  >
    <q-card-section class="row justify-start items-center q-pa-sm">
      <q-icon
        v-if="uiIconProps.name"
        v-bind="uiIconProps"
        class="q-mr-sm"
        size="sm"
      />
      <span class="text-italic information-card--title">
        {{ label }}
      </span>
    </q-card-section>
...
const uiCardProps: LinidQCardProps = ui<LinidQCardProps>(
  `${props.uiNamespace}.information-card`,
  'q-card'
);
const uiIconProps: LinidQIconProps = ui<LinidQIconProps>(
  `${props.uiNamespace}.information-card.icon`,
  'q-icon'
);
</script>
