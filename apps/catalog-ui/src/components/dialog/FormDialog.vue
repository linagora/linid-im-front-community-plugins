...
<template>
  ...
  >
    ...
    >
      <q-card
        v-bind="uiProps.card"
        :data-cy="`form-section-card`"
      >
        <q-card-section>
          <h3
            class="q-my-none form-dialog-title"
            data-cy="form_dialog_title"
          >
            {{ title }}
          </h3>
        </q-card-section>
        ...
        >
          <component
            ...
            :ui-namespace="`${uiNamespace}.form-fields-${formSection.id}`"
            :definition="field"
          />
        </q-card-section>
      </q-card>
...
const { show } = useDialog(DialogKey.Form, onOpen);
...
const uiProps = computed(() => ({
  dialog: ui<LinidQDialogProps>(`${uiNamespace.value}.form-dialog`, 'q-dialog'),
}));
